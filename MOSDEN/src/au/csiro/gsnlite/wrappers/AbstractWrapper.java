...
import au.csiro.gsnlite.windowing.LocalTimeBasedSlidingHandler;
import au.csiro.gsnlite.windowing.RemoteTimeBasedSlidingHandler;
import au.csiro.gsnlite.windowing.SlidingHandler;
import au.csiro.gsnlite.windowing.TupleBasedSlidingHandler;
...
public abstract class AbstractWrapper extends Thread {
	...
	private String TAG = "AbstractWrapper.class";
	...
	public void addListener(StreamSource ss) throws SQLException {
		...
		if (logger.isDebugEnabled())
			logger.debug(TAG, "Adding listeners: " + ss.toString());
	}
	...
	protected void postStreamElement(Serializable... values) throws Exception{
	...
	protected void postStreamElement(long timestamp, Serializable[] values) throws Exception {
	...
	protected Boolean postStreamElement(StreamElement streamElement) throws Exception {
		if (streamElement == null) {
			logger.info(TAG, "postStreamElement is called with null ! Wrapper "
		...
		try {
			...
			if (logger.isDebugEnabled())
				logger.debug(TAG, "Size of the listeners to be evaluated - "
						+ listeners.size());
	...
	public boolean insertIntoWrapperTable(StreamElement se) throws SQLException {
		...
		try {
            if (isOutOfOrder(se)) {
				logger.debug(TAG, "Out of order data item detected, it is not propagated into the system : [" + se.toString() + "]");
				return false;
			}
	...
	public StringBuilder getUselessWindow() {
		...
		synchronized (slidingHandlers) {
			for (SlidingHandler slidingHandler : slidingHandlers.values()) {
				...
				logger.debug(TAG, "***** Oldest timestamp : " + timed);
				if (timed == -1) {
					minTimed = -1;
					break;
				} else {
		...
		logger.debug(TAG, "Oldest timestamp : " + minTimed);
	...
	public int removeUselessValues() throws SQLException {
		...
		if (logger.isDebugEnabled())
			logger.debug(TAG, new StringBuilder().append(deletedRows).append(
