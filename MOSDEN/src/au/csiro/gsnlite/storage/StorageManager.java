...
import au.csiro.gsnlite.http.datarequest.AbstractQuery;
...
public abstract class StorageManager {
	...
	private static String TAG = "StorageManager.class";
    ...
    public void init(String databaseDriver, String username, String password, String databaseURL, int maxDBConnections) {
    	...
    	this.dbconnectioninfo = new DBConnectionInfo (databaseDriver,databaseURL,username,password);
        Connection con = null;       
        try {
        	        	
        	//pool = DataSources.getDataSource(new DBConnectioninfo (TAG,databaseDriver,databaseURL,username,password));
            con = getConnection();
            initDatabaseAccess(con);            
            logger.info (TAG,new StringBuilder().append("StorageManager DB connection initialized successfuly. driver:").append(databaseDriver).append(" url:").append(databaseURL).toString());
        } catch (Exception e) {
            logger.error (TAG,new StringBuilder().append("Connecting to the database with the following properties failed :").append("\n\t UserName :").append(username).append("\n\t Password : ").append(password).append("\n\t Driver class : ").append(databaseDriver).append("\n\t Database URL : ").append(databaseURL).toString());
            logger.error (TAG,new StringBuilder().append(e.getMessage()).append(", Please refer to the logs for more detailed information.").toString());
            logger.error (TAG,"Make sure in the gsn.xml file, the <storage ...> element is correct.");
            e.printStackTrace();
    ...
    * */
    ...
    public boolean tableExists(CharSequence tableName, DataField[] fields, Connection connection) throws SQLException,
        ...
        try {
            ...
            if (e.getErrorCode() == getTableNotExistsErrNo() || e.getMessage().contains("does not exist"))
                ...
                logger.error (TAG,e.getErrorCode() + "");
                throw e;
            }
        } finally {
            close(rs);
    ...
    public void shutdown() throws SQLException {
        logger.warn(TAG,"Closing the connection pool [done].");
    }
    ...
    public void executeDropTable(CharSequence tableName, Connection connection) {
        ...
        try {
            ...
            if (logger.isDebugEnabled())
                logger.debug (TAG,"Dropping table structure: " + tableName + " With query: " + stmt);
    ...
    public void executeDropView(StringBuilder tableName, Connection connection) throws SQLException {
        if (logger.isDebugEnabled())
            logger.debug (TAG,"Dropping table structure: " + tableName);
    ...
    public void executeCreateTable(CharSequence tableName, DataField[] structure, boolean unique, Connection connection) throws SQLException {
        ...
        if (logger.isDebugEnabled())
        logger.debug (TAG,new StringBuilder().append("The create table statement is : ").append(sql).toString());
        ...
        if (logger.isDebugEnabled())
            logger.debug (TAG,new StringBuilder().append(
    ...
    public DataEnumerator executeQuery(StringBuilder query, boolean binaryFieldsLinked, Connection connection) throws SQLException {
        if (logger.isDebugEnabled())
            logger.debug (TAG,"Executing query: " + query + "( Binary Field Linked:" + binaryFieldsLinked + ")");
    ...
    public DataEnumerator executeQuery(AbstractQuery abstractQuery, boolean binaryFieldsLinked, Connection connection) throws SQLException {
        ...
        String query = addLimit(abstractQuery.getStandardQuery().toString(), abstractQuery.getLimitCriterion().getSize(), abstractQuery.getLimitCriterion().getOffset());
        if (logger.isDebugEnabled())
            logger.debug (TAG,"Executing query: " + query + "(" + binaryFieldsLinked + ")");
        return new DataEnumerator(this, connection.prepareStatement(query.toString()), binaryFieldsLinked);
    }
    ...
    public void executeCreateView(CharSequence viewName, CharSequence selectQuery, Connection connection) throws SQLException {
        ...
        if (logger.isDebugEnabled())
            logger.debug (TAG,"Creating a view:" + statement);
    ...
    public void executeCommand(String sql, Connection connection) {
        ...
        try {
            ...
            logger.error (TAG,error.getMessage() + " FOR: " + sql);
        } finally {
            try {
    ...
    public void executeInsert(CharSequence tableName, DataField[] fields, StreamElement streamElement, Connection connection) throws SQLException {
        ...
        try {
            ...
            for (DataField dataField : fields) {
                ...
                switch (dataField.getDataTypeID()) {
                    ...
                    case DataTypes.BINARY:
                        ...
                        break;
                    default:
                        logger.error (TAG,"The type conversion is not supported for : "
                                + dataField.getName() + "("
            ...
            //    if (logger.isDebugEnabled())
            //        logger.debug (TAG,"An stream element dropped due to unexpected virtual sensor removal. (Stream element: " + streamElement.toString() + ")+ Query: " + query, e);
            //} else
                logger.warn(TAG,"Inserting a stream element failed : "
                        + streamElement.toString());
        } catch (SQLException e) {
            if (e.getMessage().toLowerCase().contains("duplicate entry"))
                logger.info (TAG,"Error occurred on inserting data to the database, an stream element dropped due to: " + e.getMessage() + ". (Stream element: " + streamElement.toString() + ")+ Query: " + query);
            else
                logger.warn(TAG,"Error occurred on inserting data to the database, an stream element dropped due to: " + e.getMessage() + ". (Stream element: " + streamElement.toString() + ")+ Query: " + query);
            throw e;
        }
        finally {
            close(ps);
    ...
    */
    ...
    public Connection getConnection() throws SQLException {
    	...
    	String databaseDriver = dbconnectioninfo.getDriverClass();
    	String databaseURL = dbconnectioninfo.getUrl();
