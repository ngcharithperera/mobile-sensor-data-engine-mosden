...
public class RestStreamHanlder extends HttpServlet {
	...
	private String TAG = "RestStreamHanlder.class";
	...
	public void doGet ( HttpServletRequest request , HttpServletResponse response ) throws ServletException{
        ...
        if(continuation.isExpired()){
            logger.debug(TAG, "Continuation has expired.");
            return;
        }
//prem -commented the code
//        String[] cred = null;
...
//            }
			...
			if (localContactPoint == null) {
				logger.warn(TAG, "Push streaming request received without "+PushDelivery.LOCAL_CONTACT_POINT+" parameter !");
				return;
			}
			...
			if (isExist) {
				logger.debug(TAG, "Keep alive request received for the notification-id:"+notificationId);
			...
			logger.debug(TAG, "Rest request received: "+distributionReq.toString());
			DataDistributer.getInstance(delivery.getClass()).addListener(distributionReq);
			logger.debug(TAG, "Streaming request received and registered:"+distributionReq.toString());
		}catch (Exception e) {
	...
	public void doPut( HttpServletRequest request , HttpServletResponse response ) throws ServletException  {
		...
		try {
			if (notification!=null) {
				...
				logger.warn(TAG, "Received a Http put request for an INVALID notificationId: " + notificationId);
				response.sendError(_300);
			}
		} catch (IOException e) {
			logger.warn(TAG, "Failed in writing the status code into the connection.\n"+e.getMessage(),e);
		}
	}
     ...
     */
	//AC is not used in GSN lite. so commented-Prem
