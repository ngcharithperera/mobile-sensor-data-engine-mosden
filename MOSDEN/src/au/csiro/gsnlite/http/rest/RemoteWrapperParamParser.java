...
public class RemoteWrapperParamParser {
	...
	private String TAG = "RemoteWrapperParamParser.class";
	...
	public RemoteWrapperParamParser(AddressBean addressBean, boolean isPushBased) {
		...
		logger.debug(TAG, "Remote wrapper parameter [keep-alive: "+isPushBased+"], Query=> "+query);
		...
		try {
			...
			logger.error(TAG, "Failed to parse the start-time parameter of the remote wrapper, a sample time could be:"+(CURRENT_TIME));
    ...
    public String getRemoteContactPointEncoded(long lastModifiedTime) {
		...
		logger.debug (TAG,  new StringBuilder ("Wants to connect to ").append(getRemoteContactPoint()+query+"/"+getStartTimeInString(lastModifiedTime)).append( "==Encoded==> "+toSend).toString());
		return toSend;
	}
