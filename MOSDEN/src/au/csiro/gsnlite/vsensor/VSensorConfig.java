...
import au.csiro.gsnlite.beans.WebInput;
...
public class VSensorConfig implements Serializable {
    ...
    private static String TAG = "VSensorConfig.class";
	...
	public boolean validate ( ) {
		...
		if ( mIndex < 0 && hIndex < 0 && sIndex < 0 ) {
			try {
				...
				this.logger.error (TAG, new StringBuilder( ).append( "The storage size, " ).append( storageHistorySize ).append( ", specified for the virtual sensor : " ).append( this.name )
