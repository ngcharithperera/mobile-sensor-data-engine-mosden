...
public class LocalTimeBasedSlidingHandler implements SlidingHandler {
	...
	private static String TAG = "LocalTimeBasedSlidingHandler.class";
        ...
        if (streamSource.getWindowingType() != WindowType.TIME_BASED_SLIDE_ON_EACH_TUPLE) {
            ...
            if (oldTimerTick != timerTick) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(TAG, "About to schedule new timer task at period " + timerTick + "ms in the " + wrapper.getDBAliasInStr() + " wrapper");
                }
    ...
    public long getOldestTimestamp() {
        ...
        if (maxTupleCount > 0) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(TAG, "Query for getting oldest timestamp : " + query);
            }
    ...
    private class LTBSQLViewQueryRewriter extends SQLViewQueryRewriter {
        ...
        public CharSequence createViewSQL() {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(TAG, new StringBuilder().append("The original Query : ").append(sqlQuery).toString());
