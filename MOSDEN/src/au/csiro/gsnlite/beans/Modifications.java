...
import au.csiro.gsnlite.utils.graph.Graph;
import au.csiro.gsnlite.utils.graph.Node;
import au.csiro.gsnlite.utils.graph.NodeNotExistsExeption;
...
public final class Modifications {
	...
	private static String TAG = "Modifications.class";
	...
	private void loadVirtualSensors ( Collection < String > fileNames , ArrayList < VSensorConfig > list ) {
		...
		try {
			...
			logger.error(TAG,  e1.getMessage( ) , e1 );
			return;
		}
		...
		for ( String file : fileNames ) {
			try {
				...
				//configuration = serializer.read(VSensorConfig.class, source);				
				configuration = ( VSensorConfig ) uctx.unmarshalDocument( new FileInputStream( file ) , null );
				
				configuration.setFileName( file );
				if ( !configuration.validate( ) ) {
					logger.error( TAG, new StringBuilder( ).append( "Adding the virtual sensor specified in " ).append( file ).append( " failed because of one or more problems in configuration file." )
							.toString( ) );
				...
				logger.error( TAG, new StringBuilder( ).append( "Adding the virtual sensor specified in " ).append( file ).append(
					...
					for (int addressingIndex = 0; addressingIndex < addressing.length; addressingIndex++) {
						...
						if (wrapperClass == null) {
							...
							if(logger.isDebugEnabled())
								logger.debug (TAG,  "The specified wrapper >"+addressing[addressingIndex].getWrapper()+"< does not exist");
