...
import au.csiro.gsnlite.windowing.QueryRewriter;
import au.csiro.gsnlite.windowing.WindowType;
...
public static final String DEFAULT_QUERY = "select * from wrapper";
  ...
  private static transient Logger logger             = Logger.getInstance();
	private static String TAG = "StreamSource.class";
  ...
  public boolean validate ( ) {
    ...
    isValidated=true;
    if (samplingRate <=0 ) 
      logger.warn(TAG, new StringBuilder( ).append( "The sampling rate is set to zero (or negative) which means no results. StreamSource = " )
          .append( getAlias( ) ).toString( ) );
    else if(samplingRate > 1){
    	samplingRate = 1;
    	logger.warn( TAG,new StringBuilder( ).append( "The provided sampling rate is greater than 1, resetting it to 1. StreamSource = " )
    	          .append( getAlias( ) ).toString( ) );
    }
    if (getAddressing().length==0) {
      logger.warn(TAG, new StringBuilder("Validation failed because there is no addressing predicates provided for the stream source (the addressing part of the stream source is empty)").append("stream source alias = ").append(getAlias()).toString());
    ...
    if ( this.rawHistorySize != null ) {
      ...
      if ( mIndex < 0 && hIndex < 0 && sIndex < 0 ) {
        try {
          ...
          logger.error (TAG, new StringBuilder( ).append( "The storage size, " ).append( this.rawHistorySize ).append( ", specified for the Stream Source : " ).append( this.getAlias( ) ).append(
    ...
    logger.debug (TAG,"validate() called");   
    //Parsing slide value
    if(this.rawSlideValue == null){
        ...
        if ( mIndex < 0 && hIndex < 0 && sIndex < 0 ) {
          try {
            ...
            logger.error (TAG, new StringBuilder( ).append( "The slide value, " ).append( this.rawSlideValue ).append( ", specified for the Stream Source : " ).append( this.getAlias( ) ).append(
            " is not valid." ).toString( ) );
            return (validationResult= false);
          }
        } else
  ...
  public boolean windowSlided() throws SQLException{
	  if ( logger.isDebugEnabled( ) ) 
		  logger.debug (TAG, new StringBuilder( ).append( "Data availble in the stream *" ).append( getAlias( ) ).append( "*" ).toString( ) );
...
//    if ( logger.isDebugEnabled( ) ) logger.debug (TAG, new StringBuilder( ).append( "Data avialble in the stream *" ).append( getAlias( ) ).append( "*" ).toString( ) );
...
// toReturn.append(" wrapper.timed >=").append(getStartDate().getTime()).append(" and timed <=").append(getEndDate().getTime()).append(" and ");
   ...
   // toReturn.append(" order by timed desc ");
    if ( logger.isDebugEnabled( ) ) {
      logger.debug (TAG, new StringBuilder( ).append( "The original Query : " ).append( getSqlQuery( ) ).toString( ) );
      logger.debug (TAG, new StringBuilder( ).append( "The merged query : " ).append( toReturn.toString( ) ).append( " of the StreamSource " ).append( getAlias( ) ).append(
