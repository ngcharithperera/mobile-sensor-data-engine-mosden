...
public final class Mappings {
   ...
   private static transient Logger logger             = Logger.getInstance();
	private static String TAG = "Mappings.class";
   
   public static boolean addVSensorInstance ( VirtualSensor sensorPool ) {
      try {
         if ( logger.isInfoEnabled( ) ) logger.info(TAG, ( new StringBuilder( "Testing the pool for :" ) ).append( sensorPool.getConfig( ).getName( ) ).toString( ) );
         sensorPool.returnVS( sensorPool.borrowVS( ) );
      } catch ( Exception e ) {
         logger.error(TAG, e.getMessage( ) , e );
         sensorPool.closePool( );
         logger.error(TAG, "GSN can't load the virtual sensor specified at " + sensorPool.getConfig( ).getFileName( ) + " because the initialization of the virtual sensor failed (see above exception)." );
         logger.error( TAG,"Please fix the following error" );
         return false;
      }
