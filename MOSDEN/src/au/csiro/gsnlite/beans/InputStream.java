...
import au.csiro.gsnlite.vsensor.VirtualSensor;
...
public class InputStream implements Serializable{
	...
	private static String TAG = "InputStream.class";
	...
	public void refreshAlias ( final String alias ) {
		if ( logger.isInfoEnabled( ) ) logger.info(TAG,  "REFERES ALIAS CALEED" );
	}
	...
	public boolean validate ( ) {
		...
		if (sources==null || sources.length==0) {
			logger.error(TAG, "Input Stream "+getInputStreamName()+ " is not valid (No stream sources are specified), deployment failed !");
			return false;
		}
		for ( StreamSource ss : sources ) {
			if ( !ss.validate( ) ) {
				logger.error(TAG,  new StringBuilder( ).append( "The Stream Source : " ).append( ss.getAlias( ) ).append( " specified in the Input Stream : " ).append( this.getInputStreamName( ) ).append(
	...
	public boolean executeQuery( final CharSequence alias ) throws SQLException{
		if ( logger.isDebugEnabled( ) ) logger.debug( TAG, new StringBuilder( ).append( "Notified by StreamSource on the alias: " ).append( alias ).toString( ) );
		if ( this.pool == null ) {
			logger.debug(TAG,  "The input is dropped b/c the VSensorInstance is not set yet." );
			return false;
		}

		if ( this.currentCount > this.getCount( ) ) {
			if ( logger.isInfoEnabled( ) ) logger.info(TAG,  "Maximum count reached, the value *discarded*" );
			return false;
		}

		final long currentTimeMillis = System.currentTimeMillis( );
		if ( this.rate > 0 && ( currentTimeMillis - this.lastVisited ) < this.rate ) {
			if ( logger.isInfoEnabled( ) ) logger.info( TAG, "Called by *discarded* b/c of the rate limit reached." );
			return false;
		}
		...
		if ( !queryCached ) {
			...
			//if ( logger.isDebugEnabled( ) && queryCached)
				...
				//this.rewrittenSQL ).append( "(" ).append( Main.getWindowStorage().isThereAnyResult( this.rewrittenSQL ) ).append( ")" )
				//		.toString( ) );
			/******************/
		}
		...
		if ( queryCached ){//&& Main.getWindowStorage().isThereAnyResult( this.rewrittenSQL ) ) {
			...
			if ( logger.isDebugEnabled( ) ) logger.debug(TAG,  new StringBuilder( ).append( "Executing the main query for InputStream : " ).append( this.getInputStreamName( ) ).toString( ) );
		/******************/
			/**************To Fix****/
			...
			try {
				...
				logger.warn(TAG,  "The stream element produced by the virtual sensor is dropped because of the following error : " );
				logger.warn(TAG,  e.getMessage( ) );
			} catch ( final VirtualSensorInitializationFailedException e ) {
				logger.error(TAG,  "The stream element can't deliver its data to the virtual sensor " + sensor.getVirtualSensorConfiguration( ).getName( )
				...
				try{
				this.pool.returnVS( sensor );}catch(Exception ee){
					ee.printStackTrace();
				}
		...
		if ( logger.isDebugEnabled( ) ) {
			logger.debug(TAG,  new StringBuilder( ).append( "Input Stream's result has *" ).append( elementCounterForDebugging ).append( "* stream elements" ).toString( ) );
		}
		return true;
	}
	...
	private void rewriteQuery() {
		...
		for (int i = 0; i < sources.length; i++) {
			...
			if(sb == null){
				logger.error(TAG, "Rewriting query failed. The rewrite() method of the stream source <" + sources[i].getAlias() + "> returned null.");
				rewrittenSQL = null;
				queryCached = false;
				return;
			}else{
